...
ecoli.uniprot <- read_tsv("dev/uniprot/uniprotkb_taxonomy_id_562_2025_10_10.tsv.gz") %>%
  filter(!is.na(AlphaFoldDB)) %>%
  mutate(AF_id = str_sub(AlphaFoldDB, 1, -2)) %>%
...
MG1655.gbk <- geneviewer::read_gbk("~/Work/EA_genome_prep/NC_000913.3Genes/NC_000913.3.gb")

MG1655.prot <- data.frame(GN = MG1655.gbk$NC_000913.3$FEATURES$CDS %>% map_chr(., ~.["gene"]),
                          id = MG1655.gbk$NC_000913.3$FEATURES$CDS %>% map_chr(., ~.["locus_tag"]),
                          seq = MG1655.gbk$NC_000913.3$FEATURES$CDS %>% map_chr(., ~.["translation"]),
                          stringsAsFactors = FALSE) %>%
  left_join(ecoli.uniprot.nr, by = "seq") %>%
  filter(!is.na(entry))


REL606.gbk <- geneviewer::read_gbk("~/Work/EA_genome_prep/NC_012967.1/NC_012967.1.gb")

REL606.prot <- data.frame(GN = REL606.gbk$NC_012967.1$FEATURES$CDS %>% map_chr(., ~.["gene"]),
                          id = REL606.gbk$NC_012967.1$FEATURES$CDS %>% map_chr(., ~.["locus_tag"]),
                          seq = REL606.gbk$NC_012967.1$FEATURES$CDS %>% map_chr(., ~.["translation"]),
                          stringsAsFactors = FALSE) %>%
  left_join(ecoli.uniprot.nr, by = "seq") %>%
  filter(!is.na(entry))
